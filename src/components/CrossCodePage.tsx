...
import {ProducerDetailsModal} from './producerDetailsModal/ProducerDetailsModal.tsx'
...
export const CrossCodePage = () => {
  ...
  const getProducerDisplayNameShort = (producer?: EtProducer) => {
    if (!producer) return '—'
    const name = producer.Name ?? '—'
    const prefix = producer.MarketPrefix ?? producer.Prefix ?? ''
    return prefix ? `${name} (${prefix})` : name
                ...
                treeData={[
                  {
                    ...
                    title: (
                        ...
                        actions={[
                          {
                            ...
                            onClick: () => handlePartView(tree.mainCode),
                          },
                          {
                            key: 'edit',
                            label: (
                            ...
                            onClick: () => handlePartEdit(tree.mainCode),
                          },
                          {
                            key: 'delete',
                            label: (
                            ...
                            danger: true,
                            onClick: async () => {
                              const partsPage = await fetchPartsPageWithoutProducer(tree.mainCode, 'exact')
                              const part = partsPage.items.find(p => p.Code === tree.mainCode)
                              if (part) {
                      ...
                      >
                        <div className="cross-tree-main-row">
                          <span className="cross-tree-main-row__code">
                            <b>{tree.mainCode}</b>
                          </span>
                          {tree.mainProducer && (
                            <ContextActionsMenu
                                actions={[
                                  {
                                    ...
                                    onClick: () => handleProducerView(tree.mainProducer!.Id),
                                  },
                                  {
                                    key: 'edit',
                                    label: (
                                    ...
                                    onClick: () => handleProducerEdit(tree.mainProducer!.Id),
                                  },
                                  {
                                    key: 'delete',
                                    label: (
                                    ...
                                    danger: true,
                                    onClick: () => handleProducerDelete(tree.mainProducer!),
                                  },
                                ]}
                              >
                                <span className="cross-tree-main-row__producer">
                                  {getProducerDisplayNameShort(tree.mainProducer)}
                                </span>
                              </ContextActionsMenu>
                          )}
                        </div>
                          ...
                          actions={brand.producer ? [
                            {
                              ...
                              onClick: () => handleProducerView(brand.producer!.Id),
                            },
                            {
                              key: 'edit',
                              label: (
                              ...
                              onClick: () => handleProducerEdit(brand.producer!.Id),
                            },
                            {
                              key: 'delete',
                              label: (
                              ...
                              danger: true,
                              onClick: () => handleProducerDelete(brand.producer!),
                            },
                          ] : []}
                        >
                          <span className="cross-tree-brand-row">
                            {getProducerDisplayName(brand.producer)}
                          </span>
                        </ContextActionsMenu>
                      ),
                        ...
                        title: (
                            ...
                            actions={[
                              {
                                ...
                                onClick: () => handlePartView(code.code),
                              },
                              {
                                key: 'edit',
                                label: (
                                ...
                                onClick: () => handlePartEdit(code.code),
                              },
                              {
                                key: 'delete',
                                label: (
                                ...
                                danger: true,
                                onClick: async () => {
                                  const partsPage = await fetchPartsPageWithoutProducer(code.code, 'exact')
                                  const part = partsPage.items.find(p => p.Code === code.code)
                                  if (part) {
                                    handlePartDelete(part)
                                  }
                                },
                          ...
                          >
                            <div className="cross-tree-code-row">
                              <span className="cross-tree-code-row__code">{code.code}</span>
                              <span className="cross-tree-code-row__verity">
                                {code.verity}% ({code.verity}%)
                              </span>
